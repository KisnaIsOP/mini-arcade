window.onload = function() {
    ...
    */
    ...
    function breakBlock() {
        for (var i = 0; i < blocks.length; i++) {
            if (blocks[i].getX() == boundBox.getX() && blocks[i].getY() == boundBox.getY()) {
                ...
                if (typeof MP_SendBlockUpdate === 'function') {
                    MP_SendBlockUpdate(gridX, gridY, null, 'remove');
                }
            }
        }
    }
    ...
    function placeBlock(e) {
        ...
        var gridX = Math.floor(boundBox.getX() / 32);
        var gridY = Math.floor(boundBox.getY() / 32);
        var blockType = getBlockTypeName(blockInHand);
        if (typeof MP_SendBlockUpdate === 'function') {
            MP_SendBlockUpdate(gridX, gridY, blockType, 'place');
        }
    }
    
    // Helper to convert block ID to name for multiplayer
    function getBlockTypeName(blockId) {
        switch(blockId) {
    ...
    setTimeout(function() {
        ...
        if (typeof getWidth !== 'undefined') {
            ...
            statusDiv.textContent = '✅ Game loaded! Click through the menus to start';
            ...
            statusDiv.textContent = '❌ CodeHS library failed to load - refresh the page';
